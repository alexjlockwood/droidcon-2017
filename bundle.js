...
var Command = /** @class */ (function () {
    ...
    Command.prototype.area = function () {
        if (this.type === 'M') {
            return 0;
        }
        var _a = this.start, x0 = _a[0], y0 = _a[1];
        var _b = this.end, x3 = _b[0], y3 = _b[1];
        switch (this.type) {
            case 'L':
            case 'Z':
                return (x3 - x0) * (y3 - y0);
...
function run$1() {
    ...
    var shapes = [hippo$1, elephant$1, buffalo /*, circle, star*/].map(function (d) { return Command.fromPathData(d); });
    shuffle(shapes);
    (function draw() {
        var a = shapes[0].slice(0);
        var b = shapes[1].slice(0);
...
function closestCentroids(start, end) {
    ...
    var distances = start.map(function (p1) {
        return end.map(function (p2) { return Math.pow(distance$1(centroid$1(p1), centroid$1(p2)), 2); });
    });
...
function run$4() {
    ...
    function ready(err, tx, hi) {
        ...
        function done(pieces) {
            ...
            var pairs = closestCentroids(pieces, destinations).map(function (a, i) {
                return align$2(a.slice(), destinations[i].slice());
            });
            // Collate the pairs into before/after path strings
            var pathStrings = [
                pairs.map(function (d) { return join$1(d[0]); }).join(' '),
                pairs.map(function (d) { return join$1(d[1]); }).join(' '),
            ];
            // For showing borderless when rejoined
            pathStrings.outer = join$1(points);
            svg
                .select('.mesh')
...
function run$5() {
    ...
    var path = svg.append('path');
    var circles = svg.append('g');
    var shapes = [hippo$2, elephant$2, buffalo$1, circle$3, star$1].map(function (d) { return pathStringToRing(d).ring; });
    shuffle(shapes);
    (function draw() {
        var a = shapes[0].slice(0);
        var b = shapes[1].slice(0);
        ...
        // Pick optimal winding.
        a = wind(a, b);
        path.attrs({ d: join$1(a) });
        // Redraw points.
        circles.datum(a).call(updateCircles$2);
        // Morph.
        var t = transition(undefined).duration(800);
        path
            .transition(t)
...
function runShapeToShape(options) {
    var viewportOptions = options.viewportOptions, fromData = options.fromData, toData = options.toData;
    var pixelRatio = viewportOptions.size / Math.max(viewportOptions.viewportWidth, viewportOptions.viewportHeight);
    var viewport = create$1(viewportOptions);
    var fromContainer = viewport.append('g.from');
    var toContainer = viewport.append('g.to');
    fromContainer
...
function update(container, path, data) {
...
//# sourceMappingURL=shape-to-shape-morph.js.map
...
function run$10() {
    ...
    var fromData = newOctagonData([3, 3], [6, 6]);
    var toData = newCircleData([15, 3], [18, 6]);
    var fromContainer = viewport.append('g.from');
    var toContainer = viewport.append('g.to');
    fromContainer
        .append('path.outlined')
...
function segmentsToPathDataAttr(selection) {
    selection.attr('d', function (data) {
        var bezierCommands = data
            .map(function (_a, i) {
            var h2 = _a.handleOut;
            var _b = data[(i + 1) % data.length], p = _b.segment, h1 = _b.handleIn;
...
var demoMap = new Map([
    ['/demos/intro-to-path-morphing/sq-to-sq.html', run$6],
    ['/demos/intro-to-path-morphing/sq-to-sq-morph.html', run$7],
    ['/demos/intro-to-path-morphing/sq-to-oct.html', run$8],
    ['/demos/intro-to-path-morphing/sq-with-dummies-to-oct.html', run$9],
    ['/demos/intro-to-path-morphing/sq-with-dummies-to-oct-morph.html', run$11],
    ['/demos/intro-to-path-morphing/oct-to-circle.html', run$10],
    ['/demos/intro-to-path-morphing/shift-octagon-points.html', run$14],
    ['/demos/intro-to-path-morphing/morph-sq-to-oct-reversed.html', run$12],
    ['/demos/intro-to-path-morphing/morph-sq-to-oct-shifted.html', run$13],
    ...
    ['/demos/flubber/animals-single-shape.html', run$5],
]);
